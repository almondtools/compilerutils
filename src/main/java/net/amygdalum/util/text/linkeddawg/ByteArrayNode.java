...
public class ByteArrayNode<T> implements ByteNode<T> {
	...
	public ByteArrayNode(int mask, byte[] bytes, ByteNode<T>[] nodes, byte[] alts, T attached) {
		this.mask = mask;
		this.bytes = bytes;
		this.nodes = nodes;
		this.alts = alts;
	...
	public static <T> ByteArrayNode<T> buildNodeFrom(ByteNode<T> node, NodeResolver<ByteNode<T>> resolver) {
		...
		return new ByteArrayNode<T>(mask, bytes, nodes, alts, attached);
	}
